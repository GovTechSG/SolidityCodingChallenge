...
&nbsp;
#### 1. Set up MetaMask with Ropsten
...
```
await ethernaut.methods.owner().call()
```
...
The `contract` variable is a [web3 Contract instance](https://web3js.readthedocs.io/en/1.0/web3-eth-contract.html). You may look into the web3 documentation to better understand how to interact with the smart contract from your console.

&nbsp;
#### 9. Interact with the contract to complete the level
Look into the levels's info method
```
await contract.methods.info().call()
```
You should have all you need to complete the level within the contract.

&nbsp;
#### 10. Sending transaction
To send a transaction, usually to alter states, remember to use the .send() method. 
```
await contract.methods.<contract-method>().send()
```
