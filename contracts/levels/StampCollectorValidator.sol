...
contract StampCollectorValidator {
  ...
  function submitContract(address submission) public returns (bool) {
    ...
    bool canCollectStamp = address(submission).call(
      abi.encodeWithSignature("collectStamp(address)", stamp)
    );

    // solium-disable-next-line security/no-low-level-calls
    bool canCollectNotStamp = address(submission).call(
      abi.encodeWithSignature("collectStamp(address)", notStamp)
    );
